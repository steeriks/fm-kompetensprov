...
test('semikolon skiljer namn, förband och Fmid', () => {
  const { poster } = tolkaSkyttelista('Ek, Anna; 1. plut; AnnEk05');
  assert.deepEqual(poster[0], { namn: 'Ek, Anna', forband: '1. plut', fmid: 'AnnEk05', rad: 1 });
});

test('tabb duger lika bra — så klistras ett kalkylark in', () => {
  const { poster } = tolkaSkyttelista('Ek, Anna\t1. plut\tAnnEk05');
  assert.equal(poster[0].forband, '1. plut');
  assert.equal(poster[0].fmid, 'AnnEk05');
});
...
test('överflödiga fält ignoreras i stället för att stoppa raden', () => {
  const { poster } = tolkaSkyttelista('Ek, Anna;1. plut;AnnEk05;något extra');
  assert.equal(poster[0].fmid, 'AnnEk05');
});
